...
const opticalAheadView = () => ({
  ...
  transform(code, id) {
    ...
    // Ahead is an off-axis optical view. The camera and the rendered aircraft
    // world point stay fixed while FOV provides magnification. A vertical
    // projection shift keeps the real aircraft sprite just below the viewport,
    // so only its nose is visible and all extra screen space is ahead of it.
    const modeState = `let globeCameraMode = 'free'`
    const opticalState = `let globeCameraMode = 'free'\nlet aheadFov = 40\nconst AHEAD_CAMERA_Z = 3.55\nconst AHEAD_BASE_FOV = 40\nconst AHEAD_MIN_FOV = 6\nconst AHEAD_MAX_FOV = 46\nconst AHEAD_AIRCRAFT_NDC_Y = -1.02\nconst AHEAD_AIRCRAFT_SCREEN_HEIGHT = 0.12\nconst AHEAD_TARGET_NORMAL = new THREE.Vector3(0, -0.24, 1).normalize()`
